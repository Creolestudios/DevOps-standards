...
const detectPackageManager = (cwd = process.cwd()) => {
  ...
  while (true) {
    ...
    if (fs.existsSync(path.join(dir, 'pnpm-lock.yaml'))) {
      return {
        pm: 'pnpm',
        // IMPORTANT: do NOT use `-w` here.
        // When running inside a workspace sub-package, `pnpm add -D -w` installs to the
        // workspace root, not the current package. For cs-setup we want deps on the
        // current project package that is being hooked.
        addCmd: 'pnpm add -D',
        runCmd: 'pnpm run',
