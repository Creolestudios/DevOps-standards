...
const detectPackageManager = (cwd = process.cwd()) => {
  ...
  while (true) {
    ...
    if (fs.existsSync(path.join(dir, 'pnpm-lock.yaml'))) {
      // Check if we are inside a workspace (i.e. CWD is a sub-package)
      const workspaceFile = path.join(dir, 'pnpm-workspace.yaml');
      isWorkspaceRoot = fs.existsSync(workspaceFile) && dir !== cwd;
      return {
        pm: 'pnpm',
        addCmd: isWorkspaceRoot ? 'pnpm add -D -w' : 'pnpm add -D',
        runCmd: 'pnpm run',
