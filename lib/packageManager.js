 ...
 */
let _cachedManager = null;
const detectPackageManager = () => {
  if (_cachedManager) return _cachedManager;
  
  if (fs.existsSync(path.join(process.cwd(), 'pnpm-lock.yaml'))) _cachedManager = 'pnpm';
  else if (fs.existsSync(path.join(process.cwd(), 'yarn.lock'))) _cachedManager = 'yarn';
  else if (fs.existsSync(path.join(process.cwd(), 'bun.lockb'))) _cachedManager = 'bun';
  else _cachedManager = 'npm';
...
const getPackageManagerCommand = (manager, type, pkg = '') => {
  const isPnpmWorkspace = require('fs').existsSync(require('path').join(process.cwd(), 'pnpm-workspace.yaml'));
  const pnpmInstallArgs = isPnpmWorkspace ? ['add', '-w', '-D', pkg] : ['add', '-D', pkg];
 ...
 * whitelistInPnpm()
 * 
 * Automatically adds cs-devtest to the onlyBuiltDependencies list in package.json
...
exports.whitelistInPnpm = async () => {
  ...
  try {
    ...
    if (!pkg.pnpm.onlyBuiltDependencies.includes('cs-devtest')) {
      pkg.pnpm.onlyBuiltDependencies.push('cs-devtest');
      logInfo('[pnpm] Automatically whitelisting cs-devtest for automatic installation...');
      await fs.writeJSON(pkgPath, pkg, { spaces: 2 });
      logSuccess('[pnpm] cs-devtest added to onlyBuiltDependencies in package.json.');
    }
  } catch (err) {
 ...
 * 
 * Installs all required packages for the cs-devtest functionality
 */
...
exports.installDevDependency = async (pkg) => {
  ...
  try {
    ...
    const installCmd = manager === 'npm' ? 'npm install --save-dev' : 
                      ...
                      manager === 'pnpm' ? (isPnpmWorkspace ? 'pnpm add -w -D' : 'pnpm add -D') : 'bun add -d';
    logError(
