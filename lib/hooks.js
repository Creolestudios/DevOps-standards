...
if [ "$HAS_LINT_SCRIPT" = "1" ]; then
  ...
  # If this is a pnpm workspace root, run lint for each workspace package serially.
  ...
  if [ "$IS_PNPM_WORKSPACE_ROOT" = "1" ]; then
    ...
    pnpm -r --workspace-concurrency=1 run lint --if-present
    LINT_EXIT=$?
  else
    $PKG_RUN lint
    LINT_EXIT=$?
...
# Fall back to ESLint direct run (only if we didn't already run next/turbo lint)
