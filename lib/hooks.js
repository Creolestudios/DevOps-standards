...
if [ -n "$LINT_SCRIPTS" ]; then
  ...
  if [ "$IS_PNPM_WORKSPACE_ROOT" = "1" ]; then
    ...
    STAGED_JS_FILES=$(echo "$STAGED_FILES" | grep -E '\\.js$|\\.jsx$|\\.ts$|\\.tsx$|\\.mjs$|\\.cjs$' | tr '\\n' ' ' || echo ".")
    if [ -n "$LINT_ENV" ]; then
      echo "[Lint] Legacy ESLint config detected — setting ESLINT_USE_FLAT_CONFIG=false"
      env ESLINT_USE_FLAT_CONFIG=false $PKG_RUN lint $STAGED_JS_FILES
    else
      $PKG_RUN lint $STAGED_JS_FILES
    fi
    LINT_EXIT=$?
