...
if [ "$HAS_SOURCE_CHANGES" -eq 0 ]; then
  ...
  if [ -f "./node_modules/.bin/jest" ]; then
    ./node_modules/.bin/jest --coverage --coverageReporters=lcov text --passWithNoTests --silent 2>/dev/null || true
    printf "\\033[1;32m✔ [Coverage] Jest coverage report generated\\033[0m\n"
  elif [ -f "./node_modules/.bin/vitest" ]; then
    DEBUG_PRINT_LIMIT=0 ./node_modules/.bin/vitest run --coverage --silent 2>/dev/null || true
...
if [ ! -f "sonar-project.properties" ]; then
  ...
  if grep -q "^sonar.login=REPLACE_WITH_YOUR_TOKEN" sonar-project.properties || \
    ...
    SONAR_EXIT=0
    $SONAR_BIN -Dsonar.qualitygate.wait=true -Dsonar.log.level=WARN || SONAR_EXIT=$?
