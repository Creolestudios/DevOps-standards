...
if [ "$NODE_MAJOR" -ge 22 ] 2>/dev/null && [ "$ESLINT_MAJOR" -eq 8 ] 2>/dev/null; then
  HAS_INCOMPATIBLE_PLUGIN=0
  for PLUGIN in eslint-plugin-flowtype eslint-plugin-babel eslint-plugin-standard; do
    if [ -d "./node_modules/$PLUGIN" ]; then
      HAS_INCOMPATIBLE_PLUGIN=1
      echo "⚠️  [Doctor] Node $NODE_MAJOR + ESLint v8 detected, but '$PLUGIN' is installed which is incompatible with ESLint v9."
      echo "   Skipping ESLint upgrade to avoid breaking your lint setup."
      echo "   Remove '$PLUGIN' from your project if you want to upgrade ESLint."
      break
    fi
  done
  if [ "$HAS_INCOMPATIBLE_PLUGIN" = "0" ]; then
    ...
    # Re-read ESLINT_MAJOR after upgrade so the lint runner uses the correct version
    ESLINT_MAJOR=$(node -e "try{const v=require('./node_modules/eslint/package.json').version||'';process.stdout.write(String(parseInt(v.split('.')[0],10)||0));}catch(e){process.stdout.write('0')}" 2>/dev/null)
...
if [ -n "$LINT_SCRIPTS" ]; then
  ...
  if [ "$HAS_NEXT" = "1" ]; then
    ...
    npx --no-install next lint 2>/dev/null || npx next lint
    LINT_EXIT=$?
    if [ $LINT_EXIT -ne 0 ]; then
...
if [ -f "./node_modules/.bin/jest" ]; then
  npx jest --coverage --coverageReporters=lcov text --passWithNoTests 2>/dev/null || true
