...
exports.ensureLintScriptSafety = async () => {
  ...
  // Normalize / fix up previous attempts:
  // - remove dangling `--rule` with no value (invalid)
  // - remove duplicates of the same rule override
  // - migrate legacy `--rule "no-unused-vars:off"` to ESLint v9 JSON object form
  let cleaned = lint;

  // Remove dangling --rule with no value
  cleaned = cleaned.replace(/\s--rule(\s*(?=--|$))/g, ' ');

  // Remove any existing no-unused-vars off overrides (legacy or JSON) so we can append exactly one
  cleaned = cleaned
    .replace(/\s--rule\s+["']?no-unused-vars:off["']?/g, ' ')
    .replace(/\s--rule\s+["']?\{[^"']*no-unused-vars[^"']*:\s*["']?off["']?[^"']*\}["']?/g, ' ')
    .replace(/\s--rule\s+"\{\\?"no-unused-vars\\?"\s*:\s*\\?"off\\?"\}"/g, ' ')
    .replace(/\s--rule\s+'?\{\\?"no-unused-vars\\?"\s*:\s*\\?"off\\?"\}'?/g, ' ');

  // Collapse whitespace
  cleaned = cleaned.replace(/\s+/g, ' ').trim();

  // Append a single safe override (ESLint v9 expects an object)
  const override = '--rule "{\\\"no-unused-vars\\\":\\\"off\\\"}"';
  const next = `${cleaned} ${override}`.trim();

  if (next !== lint) {
    pkg.scripts.lint = next;
    await fs.writeJSON(pkgPath, pkg, { spaces: 2 });
    logSuccess('Normalized package.json "lint" rule override for TS safety.');
  }
};
