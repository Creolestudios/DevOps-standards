 ...
 *   ONLY if the project has no ESLint plugins that are known to be incompatible
 *   with v9 (e.g. eslint-plugin-flowtype uses removed context.getAllComments API).
 * - If incompatible plugins are detected, skip the upgrade and warn instead.
 */
exports.ensureEslintRuntimeCompatibility = async () => {
  ...
  // Plugins known to be incompatible with ESLint v9 (use removed APIs like getAllComments)
  ...
  // Check if any incompatible plugins are installed in the project
  for (const plugin of v9IncompatiblePlugins) {
    const pluginPath = path.join(projectRoot, 'node_modules', plugin, 'package.json');
    if (await fs.pathExists(pluginPath)) {
      logInfo(`Node ${nodeMajor} with ESLint v8 detected, but "${plugin}" is installed which is incompatible with ESLint v9.`);
      logInfo(`Skipping ESLint upgrade to avoid breaking existing lint setup. Remove "${plugin}" first if you want to upgrade.`);
      return;
    }
  }

  // Also check if any incompatible plugins are referenced in the ESLint config
    ...
    if (await fs.pathExists(configPath)) {
      try {
        ...
        if (raw.includes('flowtype') || raw.includes('eslint-plugin-babel') || raw.includes('eslint-plugin-standard')) {
          logInfo(`Node ${nodeMajor} with ESLint v8 detected, but ESLint config references plugins incompatible with v9.`);
          logInfo(`Skipping ESLint upgrade to avoid breaking existing lint setup.`);
          return;
        }
      } catch { /* ignore */ }
    }
