...
async function checkForTypeScript(projectRoot) {
  // Primary signal: tsconfig.json is the most reliable indicator
  if (await fs.pathExists(path.join(projectRoot, 'tsconfig.json'))) {
    return true;
  }

  // Secondary signal: scan ALL top-level directories dynamically
  // Skip known non-source directories to keep it fast
  const skipDirs = new Set([
    'node_modules', '.git', '.husky', '.tools', '.github', '.vscode', '.idea',
    'dist', 'build', 'out', 'coverage', '.next', '.nuxt', '.svelte-kit',
    '.scannerwork', 'public', 'static', 'assets', 'scripts', 'tests',
  ]);

  try {
    const rootEntries = await fs.readdir(projectRoot, { withFileTypes: true });
    const sourceDirs = rootEntries
    ...
    // Ignore if root can't be read
  }

  // Also check root-level .ts/.tsx files directly
  try {
