...
exports.installHusky = async (gitRoot) => {
  ...
  const { pm } = require('./packageManager').detectPackageManager();
  const execBin =
    pm === 'pnpm' ? 'pnpm' :
    pm === 'yarn' ? 'yarn' :
    pm === 'bun'  ? 'bunx' :
  ...
  try {
    await execa(execBin, execArgs, opts);              // husky v9+
  } catch {
    try {
      await execa(execBin, [...execArgs, 'install'], opts); // husky v8 fallback
    } catch {
      logInfo("Husky init skipped — will run on next install.");
    }
  }
