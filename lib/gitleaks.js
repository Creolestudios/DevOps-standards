...
async function ensureGitignoreEntries(entries, gitRoot) {
  ...
  for (const entry of entries) {
    ...
    const normalizedEntry = entry.trim().replace(/^\*\*\//, '').replace(/^\//, '');
    const alreadyExists = content.split('\n').some(l => {
      const normalizedLine = l.trim().replace(/^\*\*\//, '').replace(/^\//, '');
      return normalizedLine === normalizedEntry && normalizedLine !== '';
    });
...
exports.installGitleaks = async (gitRoot) => {
  ...
  // Skip download if binary is already present
  if (await fs.pathExists(binaryPath)) {
    logInfo('Gitleaks already installed — skipping download.');
...
function downloadFile(url, dest) {
  return new Promise((resolve, reject) => {
    https.get(url, { headers: { 'User-Agent': 'node-cs-devtest' } }, (res) => {
