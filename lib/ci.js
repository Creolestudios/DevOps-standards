...
exports.setupCIScript = async (gitRoot) => {
  ...
  // Guard: skip if source and destination are the same file (running in cs-devtest's own dir)
...
${cdLine ? cdLine + '\n' : ''}
# ---------------------------------------------------------------
# Restore CI script if missing — using cs-devtest check-hooks
# ---------------------------------------------------------------
if [ ! -f "./scripts/run-ci-checks.sh" ]; then
  echo "⚠️  [Pre-push] CI script missing — running 'npx cs-devtest check-hooks' to restore..."
  npx cs-devtest check-hooks 2>/dev/null || true
  if [ ! -f "./scripts/run-ci-checks.sh" ]; then
    echo "❌ [Pre-push] Could not restore CI script. Skipping checks."
    exit 0
  fi
